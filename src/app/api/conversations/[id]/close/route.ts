...
export async function POST(
  ...
  // the closed state. Silently skipped for non-Gmail connections.
  if (closedConversation?.gmailThreadId) {
    void (async () => {
      const conn = await db
        ...
        .then((r) => r[0] ?? null);
      if (conn) await archiveAndLabelGmailThread(closedConversation.gmailThreadId, conn);
    })();
  }
