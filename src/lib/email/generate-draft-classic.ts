...
/**
 * Classic draft generation pipeline — preserved as the fallback path for
 * orgs whose preferred model is not Anthropic.
 *
 * This is the pre-skills implementation, moved verbatim from the original
 * generate-draft.ts so the router in generateDraftForConversation can
 * dispatch by provider. The agent-pipeline path lives in
 * src/lib/agent/pipeline.ts and is invoked by the router for Anthropic
 * orgs.
 *
 * Long-term plan: when the agent pipeline has proven itself across all
 * supported providers, this file can be removed.
 */
export async function generateDraftClassic(
